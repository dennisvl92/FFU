$NetworkShare = Read-Host "Enter network share (e.g. \\192.168.96.75\FFU-Share)"
$DriveLetter  = Read-Host "Enter drive letter (e.g. Z:)"

# Pull server from UNC: \\server\share
$server = ($NetworkShare -replace '^\\\\([^\\]+)\\.*$', '$1')

function Get-IPv4Address {
    # Parse ipconfig output to find a non-APIPA IPv4 address
    $out = ipconfig | Out-String
    $m = [regex]::Matches($out, 'IPv4 Address[^\d]*:\s*([\d\.]+)') # en-US
    if ($m.Count -eq 0) { $m = [regex]::Matches($out, 'IPv4-adres[^\d]*:\s*([\d\.]+)') } # nl-NL fallback
    foreach ($match in $m) {
        $ip = $match.Groups[1].Value
        if ($ip -and -not $ip.StartsWith("169.254.")) { return $ip }
    }
    return $null
}

function Wait-ForNetwork {
    Write-Host "Waiting for DHCP IPv4 address..."
    while (-not (Get-IPv4Address)) {
...
function Reset-WinPENetwork {
    Write-Host "Resetting WinPE network stack..."
...
while (-not $Mapped) {
    ...
    $Credential = Get-Credential -Message "Enter credentials for $NetworkShare"
    $Username   = $Credential.UserName
    $Password   = $Credential.GetNetworkCredential().Password

    # Clear possible SMB session conflicts (prevents error 1219 weirdness)
    net use \\$server\* /delete /yes 2>$null
    net use $DriveLetter /delete /yes 2>$null

    Write-Host "Mapping $DriveLetter to $NetworkShare ..."
    net use $DriveLetter $NetworkShare /user:$Username $Password
    ...
    if (Test-Path "$DriveLetter\") {
        ...
        # Important: reset network before trying again (helps when WinPE NIC/stack wedges)
        Reset-WinPENetwork
        Wait-ForNetwork
    }
}
...
function WriteLog($LogText) { 
    Add-Content -path $LogFile -value "$((Get-Date).ToString()) $LogText" -Encoding UTF8
}
...
$LogFileName = 'ScriptLog_' + $SerialNumber.SerialNumber + '.txt'
$USBDrive = $DriveLetter + '\'
New-item -Path $USBDrive -Name $LogFileName -ItemType "file" -Force | Out-Null
$LogFile = Join-Path -Path $USBDrive -ChildPath $LogFilename
...
[array]$FFUFiles = @(Get-ChildItem -Path (Join-Path -Path $USBDrive -ChildPath "FFU")*.ffu)
